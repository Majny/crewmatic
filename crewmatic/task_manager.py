...
class TaskManager:
    ...
    def reset_task(self, task_id: int, feedback: str = "") -> dict | None:
        """Reset a task back to todo (e.g. after failed verification).

        Args:
            task_id: Task to reset.
            feedback: Optional rejection feedback so the worker knows what to fix.
        """
        with self._lock:
            tasks = self._load()
            for t in tasks:
